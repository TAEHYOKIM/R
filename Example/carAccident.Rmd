...
[1] 데이터 수집
```{r}
carML <- car[c("m","주야","요일","발생지시도","사고유형_대분류","사상자수")]
str(carML)
```
...
```{r include=FALSE}
model <- lm(사상자수~사고유형_대분류+발생지시도, data = carML[-length(carML)])
summary(model)
pre <- predict(model, carML[-length(carML)])
pre
cor(carML[[length(carML)]],pre)
```
